class JSONReqService {
    ...
    request() {
        return new Promise((resolve, reject) => {
            this._request(res => {
                res.text().then(text => resolve(JSON.parse(text)));
            }, err => {
                reject(err);
            });
        });
    ...
    _printErrorLog(err) {
        console.log(`JSONReqService: request error in ${this.url}, retry:(${this.curRetryTimes}/${this.retryTimes}), error: ${err}`);
    }
